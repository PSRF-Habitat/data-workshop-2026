...
Let's start with a simple bar chart of penguin species counts
...
All figures definitely need a title, and a caption is also usually a good idea. 

```{r}
# Add in titles and caption
ggplot(penguins_clean) +
  geom_bar(aes(x = species)) +
...
Typically, we see categorical variables placed on an x-axis and numeric on the y. I, and others, argue that the rules about which variable go on which axis can be bent to improve the readability of your plot. That is unless you have date as a variable, which should always go on the x-axis. Or, if you are producing something for a very scientific audience, in which case you should stick to independent variable on the x and dependent on the y.

I always recommend putting whichever has longer axis labels on the y, especially when you have long labels, like species names, that might run into one another. You may be temped in this case to tilt the labels to 45 or even 90 degrees. Resist this urge! It is shown that a reader is much less likely to actually read the labels if they have to tilt their head to see them. 

If you cannot switch your axes for whatever reason, consider ways to shorten your axes labels before you rotate them.

Let's rotate our axes so species are on the y
...
By default, ggplot has ordered these bars alphabetically. Let's order them from highest to lowest
...
This chart would be great for knowing the relative abundance of each species. But what if we wanted to know the exact count? Adding labels to the end of bar charts is a great way to enhance accessibility 
...
All of the information we would like to share is now on our plot! Next are the steps I take with nearly every plot to make things prettier and easier to read.

The first thing to consider is how me might remove excess noise from this plot. Excess noise can come from color, lines, or text. You might notice that we have not made our species different colors in this plot - and this is intentional! Mapping variables to color should **only** come into play when you are highlighting a third variable that is not mapped to one of your axes. In this case, we could color all of the bars the same color if we wanted to for aesthetics, branding, if you really hate grey, etc. 

ggplot has a number of built in themes. I will add theme_minimal to most all my plots as a starting place for making things look nice.
...
Other places I will look for ways to remove noise is in axes labels. For general audiences, it is ok to remove these labels if the information is easily gleaned elsewhere. In this case, I will absolutely remove the x and y axes titles. I might also remove the x axis labels, since our bar labels now tell us much quicker what our penguin counts are. 
...
I also do not need the grid lines anymore
...
The last pieces I always adjust are text size, face (bold, italic, etc), position, and breathing room. Bold what you want your readers eye to draw to first. I also always like to increase margins around pretty much all text and the edges of my plot. Increasing whitespace and giving your plot room to breathe allows the reader's eyes to relax!
...
ggplot(penguins_clean |>
  ...
  labs(title = "Penguin counts at Palmer Station LTER",
       ...
       caption = "Data: Gorman et al. 2014 via {palmerpenguins}") +
  coord_flip() +
  # Base size will adjust all fonts together
  theme_minimal(base_size = 14) +
  theme(axis.title = element_blank(),
        axis.text.x = element_blank(),
        panel.grid = element_blank(),
        # I like to align the plot title with the edge of the plot
        plot.title.position = "plot",
        plot.title = element_text(face = "bold"),
        plot.margin = margin(20,20,20,20),
        plot.caption = element_text(face = "italic",
                                color = "grey50",
                                # We then change individual elements 
                                # relative to the base size
                                size = rel(0.7)))
```

Once I am happy with how my plot is looking in the plot viewer, I will save the figure to my figures folder! 

General rule is to save to the size that you will be using your figure at. Default units are inches, but this can be changed to cm, px, etc. 

```{r}
# ggsave(here("figs", "counts_bar.png"), counts_bar, width = 6)
```

After you save, open the figure from where it is saved on your computer. This is the only way to really know if you have chosen appropriate font sizes for the size you saved your image at. 12 pt font will look smaller on a 20 x 20 image than they will on a 5 x 5 image. Trial and error here is key.

This was a simple example of the iterative process of building a visualization that guides your reader to exactly what you want them to take away with no excess information. This is an easy process for making viz for a general audience, but be careful when taking away elements that are expected when creating viz for a scientific audience. This chart would look beautiful in a presentation, report, or article, but probably wouldn't fly in a scientific journal.

## Color & Highlighting

Using color tastefully and highlighting to guide your reader are great tools! 
...
Always a good idea to change from the ggplot default colors, and to increase the transparency of your data when things are overlapping.
...
Now that we have a solid graphic form and messaging, we can add our last few touches to improve aesthetics and readability by increasing margins, styling and sizing fonts, etc.
...
Let's go back in and add color to indicate the different species
...
Thinking that our trend line might not be acting as expected ... Let's make the relationships individual to each group
...
The main take away here is that it is important when you are creating analyses that you have a solid understanding about your data structure and study system!

Let's make this one pretty :) 

```{r}
ggplot(data = penguins_clean, 
       # Move aesthetics to global
       aes(x = bill_length_mm,
                 y = bill_depth_mm,
                 color = species)) +
  # Update points size and shape
  geom_point(aes(shape = species),
             size = 3,
             alpha = 0.8) +
  geom_smooth(method = "lm", se = FALSE) +
  # Change from ggplot base colors
...
1. When you can, I almost always recommend moving the legend inside of your plot, to reduce eye movement.
...
ggplot(data = penguins_clean,
  ...
  labs(title = "Penguin bill dimensions",
       ...
       shape = "Penguin species") +
  theme_minimal(base_size = 14) +
  theme(legend.position = c(0.85, 0.15), # Move legend inside plot
        plot.title.position = "plot",
        plot.margin = margin(10,10,10,10),
        axis.title.x = element_text(margin = margin(15, 0, 0, 0)),
        axis.title.y = element_text(margin = margin(0, 15, 0, 0)),
...
We can also toss out the legend entirely if there are other obvious ways to understand the groupings in the plot. One fun one is to make the subtitle text double as your legend using colored text! If you do this, keep in mind that not everyone sees color the same way. I recommend downloading an app or extension that will allow you to see your screen as someone with Deuteranopia or other forms of color blindness (On Mac, I use color oracle which does a nice job). 
...
Lastly, and perhaps the most effective, is to label your lines directly. I am a huge fan of annotations:
...
Annotations can also be a good way to call out trends or observations. Let's start a new plot, this time a scatter plot of flipper length vs body mass:
...
This is looking good! For now, to reduce eye movement and give the data more room, let's move this legend to inside the plot
...
We can use annotations to draw attention to certain pieces of the data. Let's highlight what we want our reader to take away from this plot:
...
# You might notice that if you open this saved image from your computer, the text size is not rendering as expected. This is a frustrating piece of showtext / playing with fonts. The showtext package renders the fonts at 96 DPI, which matches what the RStudio plot viewer shows us. However, we often want a higher resolution than that when we are exporting our plots. ggsave() default DPI is 300 (you can change this, but 300 DPI is standard for high quality images). 
...
# We will want to run this, then re-run our plot code, and then save once more. You will notice that things in the RStuido viewer now look pretty bad, but if we open the saved image from our computer directly, things should look pretty good! 

# Often when I am creating viz that will be saved and used in a paper, report, presentation, etc, I will save it to my computer early on and then iterate through changes, saving after each one and opening on my computer. It is certainly more time consuming, but will let you see exactly what you are going to get after saving.
